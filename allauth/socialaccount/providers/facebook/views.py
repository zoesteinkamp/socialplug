...
def fb_complete_login(request, app, token):
    info = requests.get(GRAPH_API_URL + '/me?fields=interests,likes{name,category,photos.limit(1){source}},books{name,category,photos.limit(1){source}},sports,music{photos.limit(1){source},name,category},activities{name,picture,category}',
                        params={'access_token': token.token})
    profile = requests.get(GRAPH_API_URL + '/me?fields=name,bio,gender,verified,email',
                        params={'access_token': token.token})
    info.raise_for_status()
    profile.raise_for_status()

    test = {'fb_profile': profile.json(), 'fb_info': info.json()}
    extra_data = test
