            ...
            for items in person.extra_data['spot_info']['items']:
                ...
                for k, d in music.iteritems():
                    ...
                    if Music.objects.filter(artist=artist).exists():
                        user = User.objects.get(pk=person.user.id)
                        interest = Music.objects.get(artist=artist)
                        interest.user.add(user)
                    else:
                        user = User.objects.get(pk=person.user.id)
...
def remove_fb(request, socialaccount, **kwargs):
    if socialaccount.provider == 'facebook':
        ...
        for each in interests:
            if Interest.objects.filter(name=each).exists():
                art = Interest.objects.get(name=each)
                art.user.remove(user)
            else:
                pass
    else:
        pass
...
def remove_spotify(request, socialaccount, **kwargs):
    if socialaccount.provider == 'spotify':
        artists = Music.objects.filter(user=request.user.id)
        user = User.objects.get(pk=request.user.id)
        for each in artists:

            if Music.objects.filter(artist=each).exists():
                art = Music.objects.get(artist=each)
                art.user.remove(user)
            else:
                pass
    else:
...
def remove_youtube(request, socialaccount, **kwargs):
    if socialaccount.provider == 'google':
        ...
        for each in subscriptions:
            ...
            if Subscription.objects.filter(title=each).exists():
                sub = Subscription.objects.get(title=each)
                sub.user.remove(user)
            else:
                pass
    else:
        pass